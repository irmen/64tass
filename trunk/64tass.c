...
static int toival(Obj *v1, ival_t *iv, int bits, linepos_t epoint) {
...
static int byterecursion(Obj *val, int prm, size_t *uninit, int bits, linepos_t epoint) {
    Iter *iter;
    Obj *val2;
    uint32_t ch2;
    uval_t uv;
...
static int instrecursion(List *val, int prm, int w, linepos_t epoint, struct linepos_s *epoints) {
    size_t i;
    Error *err;
    int was = 0;
    for (i = 0; i < val->len; i++) {
...
static void starhandle(Obj *val, linepos_t epoint, linepos_t epoint2) {
    ...
    do {
        if (current_section->structrecursion && !current_section->dooutput) {
                ...
                case CMD_UNION:
                    {
                        ...
                        if (labelexists) {
                            if (label->defpass == pass) {
                                ...
                                var_assign(label, &structure->v, 0);
                ...
                case CMD_VIRTUAL:
                    {
                        ...
                        str_t sectionname;
                        new_waitfor(W_ENDV2, &cmdpoint);waitfor->section=current_section;
                        ...
                        tmp->provides = ~(uval_t)0;tmp->requires = tmp->conflicts = 0;
                        tmp->dooutput = current_section->dooutput;
                        tmp->unionmode = current_section->unionmode;
                        tmp->unionstart = current_section->unionstart;
                        tmp->unionend = current_section->unionend;
                        tmp->l_unionstart = current_section->l_unionstart;
                        tmp->l_unionend = current_section->l_unionend;
                        tmp->structrecursion = current_section->structrecursion;
                        tmp->logicalrecursion = current_section->logicalrecursion;
                        tmp->file_list = cflist;
                        tmp->epoint = epoint;
                        ...
                        tmp->wrapwarn = tmp->moved = 0; star = 0;
        ...
        as_command:
            ...
            case CMD_ENDM: /* .endm */
                if (waitfor->what==W_ENDM) {
                    if (waitfor->val) ((Macro *)waitfor->val)->retval = (here() && here() != ';');
                ...
                { /* .cdef */
                    ...
                    for (;;) {
                        ...
                        case T_STR:
                            {
                                Str *str = (Str *)val;
                                if (!str->len) err_msg2(ERROR__EMPTY_STRING, NULL, &vs->epoint);
                                else {
                                    ch = str->data[0];
                                    if (ch & 0x80) i = utf8in(str->data, &ch); else i = 1;
                                    tmp.start = ch;
                                }
                                if (str->len > i) {
                                    ch = str->data[i];
                                    if (ch & 0x80) i += utf8in(str->data + i, &ch); else i++;
                                    if (tmp.start > ch) {
                                        tmp.end = tmp.start;
                                        tmp.start = ch;
                                    } else tmp.end = ch;
            ...
            case CMD_EDEF: if (waitfor->skip & 1)
                { /* .edef */
                    ...
                    for (;;) {
                        ...
                        switch (v->obj->type) {
                        case T_STR:
                            if (!((Str *)v)->len) err_msg2(ERROR__EMPTY_STRING, NULL, &vs->epoint);
                            break;
                        case T_NONE:
                            err_msg_still_none(NULL, &vs->epoint);
                            tryit = 0;
            ...
            case CMD_CPU: if (waitfor->skip & 1)
                { /* .cpu */
                    ...
                    case T_STR:
                        ...
                        len = ((Str *)val)->len;
                        while (cpui->name) {
                            if (len == strlen(cpui->name) && !memcmp(cpui->name, ((Str *)val)->data, len)) {
            ...
            case CMD_VIRTUAL: if (waitfor->skip & 1)
                { /* .virtual */
                    ...
                    str_t sectionname;
                    listing_line(0);
                    new_waitfor(W_ENDV2, &epoint);waitfor->section=current_section;
                    if (sizeof(anonident2) != sizeof(anonident2.type) + sizeof(anonident2.padding) + sizeof(anonident2.star_tree) + sizeof(anonident2.vline)) memset(&anonident2, 0, sizeof(anonident2));
                    else anonident2.padding[0] = anonident2.padding[1] = anonident2.padding[2] = 0;
                    anonident2.type = '.';
                    anonident2.star_tree = star_tree;
