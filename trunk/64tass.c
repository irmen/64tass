...
static unsigned int nolisting;   /* listing */
address_t star=0;
    ...
    while (nobreak) {
        ...
        if (labelname.len) {
            ...
            if (wht == '=') { /* variable */
                ...
                if (!nolisting && flist && arguments.source && label->ref) {
                    int l;
            ...
            if (wht == '.') { /* .proc */
                ...
                case CMD_SECTION:
                    ...
                    if (newlabel->ref && !nolisting && flist && arguments.source) {
                        int l;
                        l = printaddr('.', current_section->address, LIST_CODE);
                        if (labelname.len) {
                            while (l < 40) { l += 8; putc('\t', flist); }
        ...
        case '*':if (waitfor->skip & 1) /* skip things if needed */
            {
                ...
                if (current_section->structrecursion && !current_section->dooutput) err_msg(ERROR___NOT_ALLOWED, "*=");
                    ...
                    struct value_s err;
                    uval_t uval;
                    if (touval(val, &err, &uval, 8*sizeof(uval_t), &epoint)) { err_msg_output_and_destroy(&err); break; }
        ...
        as_command:
            ...
            case CMD_LOGICAL: if (waitfor->skip & 1)
                { /* .logical */
                    ...
                    if (current_section->structrecursion && !current_section->dooutput) err_msg2(ERROR___NOT_ALLOWED, ".LOGICAL", &opoint);
                        ...
                        if (touval(val, &err, &uval, (all_mem == 0xffff) ? 16 : 24, &epoint)) err_msg_output_and_destroy(&err); 
            ...
            case CMD_REPT: if (waitfor->skip & 1) 
                ...
                break;
            case CMD_PRON: /* .pron */
                if ((waitfor->skip & 1) && nolisting) nolisting--;
                break;
            case CMD_PROFF: /* .proff */
                if (waitfor->skip & 1) nolisting++;
                break;
        ...
        case '#':if (waitfor->skip & 1) /* skip things if needed */
            ...
            as_macro:
                if (!nolisting && flist && arguments.source && wasref) {
                    int l;
                    l = printaddr('.', current_section->address, LIST_CODE);
                    if (labelname.len) {
                        while (l < 40) { l += 8; putc('\t', flist); }
