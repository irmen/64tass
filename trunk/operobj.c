...
Oper o_MIN         = { {&obj, 0}, "smaller of '<?", O_MIN, 9};
Oper o_MAX         = { {&obj, 0}, "greater of '>?", O_MAX, 9};
Oper o_OR          = { {&obj, 0}, "binary or '|", O_OR, 10};
Oper o_XOR         = { {&obj, 0}, "binary exclusive or '^", O_XOR, 11};
Oper o_AND         = { {&obj, 0}, "binary and '&", O_AND, 12};
Oper o_LSHIFT      = { {&obj, 0}, "binary left shift '<<", O_LSHIFT, 13};
Oper o_RSHIFT      = { {&obj, 0}, "binary right shift '>>", O_RSHIFT, 13};
Oper o_ADD         = { {&obj, 0}, "add '+", O_ADD, 14};
Oper o_SUB         = { {&obj, 0}, "subtract '-", O_SUB, 14};
Oper o_MUL         = { {&obj, 0}, "multiply '*", O_MUL, 15};
Oper o_DIV         = { {&obj, 0}, "division '/", O_DIV, 15};
Oper o_MOD         = { {&obj, 0}, "modulo '%", O_MOD, 15};
Oper o_EXP         = { {&obj, 0}, "exponent '**", O_EXP, 16};
Oper o_NEG         = { {&obj, 0}, "unary negative '-", O_NEG, 17};
Oper o_POS         = { {&obj, 0}, "unary positive '+", O_POS, 17};
Oper o_INV         = { {&obj, 0}, "binary invert '~", O_INV, 17};
Oper o_LNOT        = { {&obj, 0}, "logical not '!", O_LNOT, 17};
Oper o_COMMAX      = { {&obj, 0}, "register indexing ',x", O_COMMAX, 17};
Oper o_COMMAY      = { {&obj, 0}, "register indexing ',y", O_COMMAY, 17};
Oper o_COMMAZ      = { {&obj, 0}, "register indexing ',z", O_COMMAZ, 17};
Oper o_COMMAS      = { {&obj, 0}, "register indexing ',s", O_COMMAS, 17};
Oper o_COMMAR      = { {&obj, 0}, "register indexing ',r", O_COMMAR, 17};
Oper o_COMMAD      = { {&obj, 0}, "register indexing ',d", O_COMMAD, 17};
Oper o_COMMAB      = { {&obj, 0}, "register indexing ',b", O_COMMAB, 17};
Oper o_COMMAK      = { {&obj, 0}, "register indexing ',k", O_COMMAK, 17};
Oper o_SPLAT       = { {&obj, 0}, "unary splat '*", O_SPLAT, 18};
Oper o_CONCAT      = { {&obj, 0}, "concatenate '..", O_CONCAT, 19};
Oper o_X           = { {&obj, 0}, "repeat 'x", O_X, 20};
Oper o_MEMBER      = { {&obj, 0}, "member '.", O_MEMBER, 21};
